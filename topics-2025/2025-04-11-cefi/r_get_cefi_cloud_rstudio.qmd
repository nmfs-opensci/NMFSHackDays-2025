...
## reticulate for running Python in R
...
There are 2 approaches and which one you use will depend on the environment you are using. Pick the appropriate one and run the code for that case (and only for that case). [See reticulate setup](https://nmfs-opensci.github.io/NMFSHackDays-2025/topics-2025/2025-04-11-cefi/reticulate_setup.html) for more details.

#### You installed R with conda
...
#### You installed R normally (not using `conda install r`)
...
Use `py_require()` to create an ephemeral environment with your packages. Note there is a problem with `zarr 3.*` and opening zarr files, so you need to make sure `zarr<3` is installed.

```{r}
reticulate::py_require(c("zarr<3", "fsspec", "xarray", "gcsfs", "s3fs"))
```
